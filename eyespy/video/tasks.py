...
import os, cv2, argparse, csv, numpy as np
import time as tm 
...
def process_video(self, filename):
    ...
    path_to_file_folder = media_url + "/video/20" + filename + ".mp4"
    #To find total number of frames
    cap = cv2.VideoCapture(path_to_file_folder)
    no_of_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    print(f"Processing {no_of_frames} frames")
    ...
    while(vid.isOpened()):
        ...
        if(ind%FRAMES_PER_DETECTION == 0):
            ...
            #break
        ind += 1
        desc = f'Processed {ind}/{no_of_frames}'
        progress_recorder.set_progress(ind, no_of_frames, description=desc)
